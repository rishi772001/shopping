...
exports.login = async (req, res) => {
  try {
    ...
    if (data === null || data === {}) {
      res
        .status(200)
        .send({ success: true, loggedIn: false, msg: "Invalid Email" });
    } else if (data.password !== req.body.password) {
      res
        .status(200)
        .send({ success: true, loggedIn: false, msg: "Invalid Password" });
    } else {
      res
        .status(200)
        .send({ success: true, loggedIn: true, msg: "Logged in", user: data });
    }
  } catch (err) {
    res.status(404).send({ success: false, msg: err.message });
  }
...
exports.getUser = async (req, res) => {
  try {
    let data = await UserModel.findById(req.params.id).populate({
      path: "purchases",
      model: "Purchase",
      populate: {
        path: "products",
